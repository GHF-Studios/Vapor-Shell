...
mod tests {
    ...
    fn temp_root(label: &str) -> PathBuf {
        let root = env::temp_dir().join(format!(
            "vapor-diagnostics-{label}-{}-{}",
            std::process::id(),
            now_parts().nanos
        ));
        fs::create_dir_all(root.join("bin")).expect("create app bin");
        fs::write(
            root.join(manifest::APP_FILE_NAME),
            "[root]\nname = \"test-app\"\n",
        )
    ...
    #[test]
    fn redacts_obvious_secrets_from_args_and_text() {
        ...
        assert_eq!(
            redact_text("authorization: bearer-secret public"),
            "authorization:<redacted> <redacted> public"
        );
    }

    #[test]
    fn structured_run_writes_metadata_log_and_latest() {
        let root = temp_root("structured");
        let executable = root.join("bin/vapor");
        ...
        .expect("prepare diagnostics run");
        assert!(run.local.run_dir().is_dir());
        assert!(run.local.metadata_path().is_file());
        assert!(run.local.log_path().is_file());
        let metadata =
            fs::read_to_string(run.local.metadata_path()).expect("read diagnostics metadata");
        assert!(metadata.contains("schema = 1"), "{metadata}");
        assert!(metadata.contains("metadata.toml"), "{metadata}");
        assert!(metadata.contains("vapor.log"), "{metadata}");
        assert!(metadata.contains("short-random"), "{metadata}");
        assert!(!metadata.contains("machine-id"), "{metadata}");
        assert!(!metadata.contains("hostname"), "{metadata}");
        assert!(!root.join(".vapor/state/diagnostics/machine-id").exists());
        let latest = fs::read_to_string(root.join(".vapor/diagnostics/latest.toml"))
            .expect("read latest pointer");
        assert!(latest.contains("run-dir"), "{latest}");
        assert!(latest.contains(run.local.run_id()), "{latest}");
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn unix_date_matches_expected_boundaries() {
        assert_eq!(utc_date(0), "1970-01-01");
        assert_eq!(utc_date(86_400), "1970-01-02");
        assert_eq!(utc_date(1_783_209_600), "2026-07-05");
    }
}
